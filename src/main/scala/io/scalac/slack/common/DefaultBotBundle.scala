...
import io.scalac.slack.bots.bob.BobBot
...
import io.scalac.slack.bots.{DirectMessageTestBot, ImportantMessageBot, LoggingBot, RichMessageTestBot}
...
//    val votingtBot = context.actorOf(Props(classOf[VotingBot], new InMemoryVotingRepo, bus), "votingBot")
    val bobBot = context.actorOf(Props(classOf[BobBot], bus), "BobBot")
  }
}
